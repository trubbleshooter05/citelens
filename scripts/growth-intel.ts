...
const UGC_VOICE =
  "TikTok creator ad read — punchy hook, raw founder energy, micro-pauses between beats, like a one-take vertical video";
...
function buildUgcCreative(target: TargetCluster, date: string, history: UgcHistoryItem[]): Pick<Opportunity, "ugcVoice" | "ugcHook" | "ugcScript" | "ugcReferenceVideo"> {
  ...
  const hooks = [
    `POV: you finally searched "${kw}"… and realized AI answers don't cite you. That sting is the whole point.`,
    `Stop. If you're only watching Google for "${kw}", you're auditing the wrong surface — AI already decided.`,
    `Nobody warned me "${kw}" was a different game than classic SEO. So I built the weekly fix list anyway.`,
  ];
  const bridges = [
    "Here's what bugs me about most GEO tools — they stop at a score. Cool chart. Zero shipped copy.",
    "If a keyword can't become one tactical paragraph your team can publish this week, I don't want it in the workflow.",
    "I'm not hunting vanity metrics. I want a queue: one citation gap, one paragraph you can paste into the page.",
    "The bar is stupid simple: does this turn into a shipped change, or is it just another dashboard tab?",
  ];
  const closes = [
    "Demo's live — roast it. If the task brief feels real, I build live audits next. If it's fluff, I kill it.",
  ...
  for (let i = 0; i < hooks.length * bridges.length * closes.length; i += 1) {
    ...
    script = `${hook} ${bridge} CiteLens compresses that gap into one copy-ready move: ${target.nextAction} Not more charts — one execution. ${close}`;
...
function buildMarkdown(date: string, routes: Set<string>, rows: CsvRow[], opportunities: Opportunity[], freshnessWarning: string): string {
  const top = opportunities; // all opportunities exposed so daily-ugc-workflow can rotate by date
